...
<div id="modal" class="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
    <div class="bg-white p-6 rounded-lg shadow-lg w-[50rem] relative">
        
        <!-- Close Button -->
        <div class="flex items-center justify-between">
        <h2 class="text-xl font-semibold mb-4">Records on {{ \Carbon\Carbon::parse($date)->format('F j, Y') }}</h2>
        <button onclick="document.getElementById('transaction-add-{{ \Carbon\Carbon::parse($date)->format('j') }}').classList.remove('hidden')" 
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($transactions as $transaction)
                        <tr title="Click this to preview {{$transaction->item->name}}"
                            class="cursor-pointer hover:bg-gray-300" onclick="document.getElementById('single-preview-{{$transaction->id}}').classList.remove('hidden')">
        ...
        <div class="flex justify-end mt-2">
            <button onclick="document.getElementById('modal').classList.add('hidden')" class="px-4 py-2 text-gray-100 bg-gray-500 hover:bg-gray-800 rounded">Close</button>
        </div>
    </div>
</div>
