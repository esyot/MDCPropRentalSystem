    ...
    <div class="onload:scale-110 bg-white p-6 rounded-lg shadow-lg w-full max-w-4xl relative transform scale-[500px] transition-all duration-[500ms] ease-in-out">
        <form action="{{ route('transaction-create') }}" method="POST">
            ...
            <button 
                ...
                onclick="document.getElementById('transaction-add-{{$day}}').classList.add('hidden')">
                &times;
            </button>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <!-- Rentee Contact No -->
                <div>
                    <label for="rentee_contact_no" class="block text-gray-700 font-semibold mb-1">Rentee Contact No</label>
                    <input 
                        id="rentee_contact_no"
                        name="rentee_contact_no" 
                        type="text" 
                        class="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                        placeholder="Rentee Contact No" 
                        required>
                </div>
                ...
                <!-- Rent Return Time -->
                <div>
                    <label for="rent_return_time" class="block text-gray-700 font-semibold mb-1">Rent Return Time</label>
                    <input 
                        id="rent_return_time"
                        name="rent_return_time"
                        type="time" 
                        class="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                        required>
                </div>
            ...
            <div class="col-span-2 flex justify-end mt-4 space-x-2">
                ...
                <button type="button" onclick="document.getElementById('transaction-add-{{$day}}').classList.add('hidden')" class="px-4 py-2 bg-gray-500 hover:bg-gray-800 text-gray-100 rounded">
                    Cancel
                </button>
            </div>
        </form>
...
@if(session('success'))
    ...
    <div id="successModal" class="fixed inset-0 flex items-center justify-center z-50 hidden">
        <div class="bg-green-500 text-white p-4 rounded mb-4">
            {{ session('success') }}
            <button onclick="document.getElementById('successModal').classList.add('hidden')" class="ml-4 bg-green-700 hover:bg-green-800 text-white font-bold py-2 px-4 rounded">
                Close
            </button>
        </div>
    </div>
