<div id="single-preview-{{$transaction->id}}" class="bg-gray-800 bg-opacity-50 fixed inset-0 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-lg shadow-lg w-[35rem] max-w-full relative overflow-hidden">
        <!-- Close Button -->
        <button class="absolute top-2 text-2xl right-4 text-gray-500 hover:text-gray-700" onclick="document.getElementById('single-preview-{{$transaction->id}}').classList.add('hidden')">&times;</button>

        <!-- Modal Content -->
        <div class="flex flex-col">
            <!-- Image Section -->
            <div class="w-full">
                <img src="{{$transaction->item->img}}" alt="Product Image" class="w-full h-60 object-cover rounded-t-lg">
            </div>
