...
<div id="content" class="flex items-center justify-between  p-4 shadow-md">
    <div class="flex items-center space-x-2">
        <form id="filter-form" class="flex justify-around space-x-4" action="{{ route('dateCustom')}}" method="GET">
            ...
            @foreach($currentCategory as $category)
                <a
                    href="{{ route('calendarMove', ['action' => 'left', 'category' => $category->id, 'year' => $currentDate->format('Y'), 'month' => $currentDate->format('n')])}}">
                    <i
            ...
            @foreach($currentCategory as $category)
                <a
                    href="{{ route('calendarMove', ['action' => 'right', 'category' => $category->id, 'year' => $currentDate->format('Y'), 'month' => $currentDate->format('n')])}}">
                    <i
...
<div id="main-content" class="p-6 flex-1 overflow-auto w-full h-screen">
    <div id="calendar-grid" class="grid grid-cols-7 gap-2 p-2 h-full shadow-lg">
