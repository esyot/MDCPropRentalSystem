...
<div id="content" class="flex items-center justify-between bg-gradient-to-r from-cyan-500 to-cyan-800 p-4 shadow-md">
    <div class="flex items-center space-x-2">
        <form id="filter-form" class="flex justify-around space-x-4" action="" method="GET">
            @csrf
...
<div class="p-6 bg-white flex-1 overflow-auto w-full h-full">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4">
        ...
        <div class="bg-white rounded-lg shadow-inner overflow-hidden">
            <img src="{{ $transaction->item->img }}" alt="{{ $transaction->item->name }}" class="w-full h-48 object-cover">
            <div class="p-4 bg-gradient-to-r from-teal-500 to-teal-800 text-white">
...
@foreach($transactions as $transaction)
<div id="delete-confirmation-{{$transaction->id}}" class="hidden fixed inset-0 flex items-center justify-center z-50 bg-gray-800 bg-opacity-50 confirm-dialog">
    <div class="relative px-4 min-h-screen md:flex md:items-center md:justify-center">
        <div class="bg-white p-6 rounded-lg md:max-w-md md:mx-auto p-4 fixed inset-x-0 bottom-0 z-50 mb-4 mx-4 md:relative shadow-lg">
            ...
            <div class="flex items-center mt-1">
                ...
                <span class="ml-2 text-red-500">
                    ...
                    <div class="flex justify-center m-2 space-x-2">
                        <a href="{{ route('transactionDecline', ['id'=>$transaction->id]) }}" id="confirm-delete-btn" class="hover:bg-red-400 w-full md:w-auto px-4 py-3 md:py-2 bg-red-200 text-red-700 rounded-lg font-semibold text-sm md:ml-2">
                            Yes, decline.
                        </a>
                        <button onclick="document.getElementById('delete-confirmation-{{$transaction->id}}').classList.add('hidden')"
