...
<body class="flex h-screen overflow-hidden bg-gray-100 text-gray-800">
 ...
 <!-- Sidebar -->
<div id="sidebar-right" class="shadow-md fixed top-0 right-0 h-full w-64 bg-white transform translate-x-full z-50 {{ $setting->transition == true ? 'transition-transform duration-[500ms] ease-in-out' : '' }}">
    <div class="p-4">
        ...
        <form action="{{ route('darkMode') }}" method="POST">
            ...
            <div class="flex items-start mt-4">
                <div class="px-3 py-1 m-2 text-4xl bg-gray-500 rounded-full">
        ...
        <form action="{{ route('transitions') }}" method="POST">
            @csrf
            <div class="flex items-start mt-4">
                <div class="px-2 py-1 m-2 text-4xl bg-gray-500 rounded-full">
                    <div class="text-white fa-solid fa-arrows-left-right"></div>
                </div>
                <div class="flex flex-col flex-wrap">
                    <p class="font-bold">Transitions</p>
                    <p>Smooth out the transition animations, even if it results in slower system performance.</p>
                    <div class="flex items-center justify-between mt-2">
                        <label class="slider">
                            <input 
    ...
    <!-- Sidebar -->
    <div id="sidebar" class="bg-gradient-to-b from-blue-500 to-blue-800 h-full flex flex-col text-white shadow-lg w-20 {{ $setting->transition == true ? 'transition-all duration-[300ms] ease-in-out' : '' }}">
        <!--  -->
